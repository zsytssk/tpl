...
export function convertXMLToNode(xmlText) {
    ...
    const node = convertJSONToNode(jsonObj);
    return node;
}
export function convertJSONToNode(jsonObj) {
    const type = jsonObj.type;
    const TypeCtor = ClassUtils.getClass(type);
    if (!TypeCtor) {
        return;
    }
    const node = new TypeCtor();
...
export function addPressEvent(sprite) {
    /**  按下, 开始记录  */
    sprite.on(Event.MOUSE_DOWN, sprite, (event) => {
        sprite.cus_event = {
            stageX: event.stageX,
            stageY: event.stageY,
            status: 'MOUSE_DOWN',
    ...
    /**  取消 */
    sprite.on(Event.MOUSE_MOVE, sprite, () => {
        if (!sprite.cus_event) {
            return;
        }
        sprite.cus_event = {
    ...
    sprite.on(Event.MOUSE_UP, sprite, (event) => {
        if (!sprite.cus_event) {
            return;
        }
